...
#
# 用法：bash .codex/skills/UAT/resources/full-protocol-uat.sh
set -uo pipefail
...
# MCP 配置（run --mcp-config 期望顶层 "servers" 数组）。两个 stdio server：
...
cat > "$U/cfg.json" <<EOF
{"servers":{
  "echo":{"type":"stdio","disabled":false,"server_parameters":{"command":"node","args":["$MCP"]}},
  "no-resources":{"type":"stdio","disabled":false,"server_parameters":{"command":"node","args":["$MCP_NORES"]}}
}}
EOF
