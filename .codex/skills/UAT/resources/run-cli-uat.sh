...
SKILL_DIR="$(cd "$(dirname "$0")/.." && pwd)"          # .codex/skills/UAT
...
mkdir -p "$U/project"
o=$(cd "$U/project" && "$A2C" settings set x y --scope project --json 2>&1)
if [[ "$?" -eq 0 ]] && echo "$o" | grep -q '"scope": "project"' && [[ -f "$U/project/.tfrobot/settings.json" ]]; then
  PASS=$((PASS+1)); printf '  ✅ %s\n' G-05
else
  FAIL=$((FAIL+1)); FAILED+=("G-05"); printf '  ❌ %s (project scope must anchor to isolated cwd)\n' G-05
fi
...
o=$("$A2C" plugin list --available --json 2>&1); ck P-02 0 $? '"enabled": false' "$o"
