...
### G-05: settings set --scope project（锚定当前工作目录）

- **优先级**: P1
- **步骤**:
  1. 新建隔离目录 `$U/project`；
  2. 在该目录执行 `$A2C settings set strictKnownMarketplaces true --scope project --json`。
- **预期结果**:
  - 退出码 0；
  - 输出包含 `"scope": "project"`；
  - 只写入 `$U/project/.tfrobot/settings.json`，不污染仓库或用户配置。

> #98 后 `--add-dir` 已移除，project/local scope 直接锚定进程 cwd。
