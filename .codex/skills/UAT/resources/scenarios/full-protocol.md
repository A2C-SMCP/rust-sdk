...
|---|---|---|
| Server | `uv run python ... _run_server_process` | `target/debug/examples/uat_test_server 127.0.0.1:0`（仅 UAT 的 permissive server）|
| Computer | `uv run a2c-computer run --url ...` | `target/debug/smcp-computer --url http://127.0.0.1:<PORT> run --mcp-config <cfg>` |
...
> `SMCP_COMPUTER`、`SMCP_TEST_MODE`（`get_tools` 执行 F-08）。
...
1. 三个二进制已编译：
   ...
   ```
2. Computer 至少挂载一个 MCP server（有工具 + 资源）。canonical 编排使用
   `tests/v022-mcp-server/index.js` 和 `tests/no-resources-mcp-server/index.js`。
3. （多用例）tmux MCP 工具可用。
...
```json
{"servers":{
  "echo":{"type":"stdio","disabled":false,
    "server_parameters":{"command":"node","args":["tests/v022-mcp-server/index.js"]}}
}}
```
...
> **一键编排**：`bash .codex/skills/UAT/resources/full-protocol-uat.sh`——三进程真实链路，
...
- **优先级**: P0
- **步骤**: 启动 Server → 启动 Computer → Computer REPL `socket join proto-uat-office friday_hands`
- **预期结果**:
...
- **步骤**: 运行 agent：
  ...
  SMCP_AGENT_ID=agent1 SMCP_COMPUTER=friday_hands SMCP_TEST_MODE=get_tools \
  cargo run -p smcp-agent --example e2e_test_agent
  ```
- **预期结果**:
  - Agent 日志 `UAT_RESULT: PASS get_tools`，列出 `echo__echo`
  - 无 timeout / error

### F-02: tool_call 路由 ✅

- **驱动**: `SMCP_TEST_MODE=call_tool`
- **步骤**: agent `tool_call(computer, "echo__echo", {message:"hello-uat"})`
