...
- **预期结果**（已对照 Rust 实际输出）:
  ...
  - 安装只建立 durable intent；`enabledPlugins` 缺省时保持 disabled，须由 P-05 显式启用

### P-02: plugin list --available（列出已安装但未启用）

- **优先级**: P0 / **前置**: P-01
- **步骤**:
  1. `$A2C plugin list --json`
  2. `$A2C plugin list --available --json`
- **预期结果**:
  - 两条均退出码 0
  - 默认 `list` 返回 `[]`
  - `list --available` 数组含 `{"id": "foo@mp-bundled-mcp", "enabled": false, "scopes": ["user"], "bundledMcpServers": ["figma-mcp"]}`
