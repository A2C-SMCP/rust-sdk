...
SEEDS_ROOT=<项目根>/.codex/skills/UAT/resources/seeds
U="/tmp/a2c-uat-$$"
...
- **状态**: ✅ `full-protocol-uat.sh` 已跑通
- **引用 seed**: `seeds/_common/valid-skill-pkg`
- **预期行为**:
...
- **完整链路实现**：`full-protocol-uat.sh` 将 `valid-skill-pkg` seed 放入隔离 user
  source，启动 `uat_test_server`、Computer 和 `e2e_test_agent`，由 `skill_disclosure`
  mode 验证 `get_skills` → `get_skill` 及子资源链路。
