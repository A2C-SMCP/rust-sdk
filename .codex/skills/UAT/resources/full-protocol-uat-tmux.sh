...
#
#   脚本启动后按输出的唯一 session 名 attach；切 window：Ctrl-b 0/1/2（server/computer/agent）
#
...
# 成功自动 kill。用法：bash .codex/skills/UAT/resources/full-protocol-uat-tmux.sh
set -uo pipefail
...
SESSION="${A2C_UAT_TMUX_SESSION:-a2c-uat-fp-$$}"

command -v tmux >/dev/null || { echo "missing tmux"; exit 1; }
if tmux has-session -t "$SESSION" 2>/dev/null; then
  echo "tmux session already exists; refusing to replace it: $SESSION"
  exit 1
fi
...
printf '%s\n' "{\"servers\":{\"echo\":{\"type\":\"stdio\",\"disabled\":false,\"server_parameters\":{\"command\":\"node\",\"args\":[\"$MCP\"]}},\"no-resources\":{\"type\":\"stdio\",\"disabled\":false,\"server_parameters\":{\"command\":\"node\",\"args\":[\"$MCP_NORES\"]}}}}" > "$U/cfg.json"
...
tmux send-keys -t "$SESSION:computer" "source $U/env.sh && '$COMP' --url http://127.0.0.1:$PORT --approve-all-mcp run --mcp-config $U/cfg.json 2>&1 | tee $U/computer.log" Enter
