...
| `a2c-computer run --url ...` | `$A2C --url ... run` |
...
```bash
bash .codex/skills/UAT/resources/run-cli-uat.sh
...
bash .codex/skills/UAT/resources/full-protocol-uat-tmux.sh
...
bash .codex/skills/UAT/resources/full-protocol-uat.sh
```
...
> Rust 端进程：Server = `uat_test_server` example（permissive，仅 UAT）；Computer =
> `$A2C --url ... run --mcp-config ...`；Agent = `e2e_test_agent` example。
...
- scenario: （可选）测试场景名称（如 marketplace-ops、oauth-atlassian）。不填则执行默认全量；
  `oauth-atlassian` 仅显式指定时执行。
