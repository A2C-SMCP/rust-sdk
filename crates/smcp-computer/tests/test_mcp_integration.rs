...
async fn test_stdio_client_with_echo_server() {
    ...
    let params = StdioServerParameters {
        ...
        args: vec![
            "-c".to_string(),
            r#"echo '{"jsonrpc":"2.0","id":0,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{"listChanged":true}},"serverInfo":{"name":"mock","version":"0.1.0"}}}'; while IFS= read -r line; do id=$(echo "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p'); [ -n "$id" ] && echo "{\"jsonrpc\":\"2.0\",\"id\":$id,\"result\":{\"tools\":[]}}"; done"#.to_string(),
        ],
        env: HashMap::new(),
        cwd: None,
    };
