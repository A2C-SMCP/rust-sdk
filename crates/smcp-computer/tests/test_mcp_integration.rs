...
async fn test_stdio_client_with_echo_server() {
    ...
    let params = StdioServerParameters {
        ...
        args: vec!["-c".to_string(), "echo '{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"tools\":[]}}'; cat".to_string()],
        env: HashMap::new(),
        cwd: None,
    };
    ...
    assert!(connect_result.is_ok(), "Connection should succeed with valid JSON response");
    ...
    assert!(tools.is_empty(), "Tools list should be empty for mock server");
...
async fn test_http_client_connection_timeout() {
    ...
    assert!(connect_result.is_ok(), "Connection should timeout within 35 seconds");
...
async fn test_sse_client_invalid_url() {
    ...
    for url in invalid_urls {
        ...
        let connect_result = client.connect().await;
        assert!(connect_result.is_err(), "Connection should fail with invalid URL: {}", url);
    }
...
async fn test_client_parameter_serialization() {
    ...
    assert_eq!(deserialized.headers.get("Authorization").unwrap(), "Bearer token123");
