...
async fn test_cache_with_multiple_inputs() {
    ...
    computer
        .set_input_value(
            "prompt1",
            serde_json::Value::String("cached_prompt".to_string()),
        )
        .await
        .unwrap();
    computer
        .set_input_value("pick1", serde_json::Value::String("opt2".to_string()))
        .await
        .unwrap();
    computer
        .set_input_value(
            "cmd1",
            serde_json::Value::String("cached_output".to_string()),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        values.get("prompt1"),
        Some(&serde_json::Value::String("cached_prompt".to_string()))
    );
    assert_eq!(
        values.get("pick1"),
        Some(&serde_json::Value::String("opt2".to_string()))
    );
    assert_eq!(
        values.get("cmd1"),
        Some(&serde_json::Value::String("cached_output".to_string()))
    );
}
...
async fn test_cache_concurrent_access() {
    ...
    let set_task = tokio::spawn(async move {
        for i in 0..10 {
            ...
            computer_clone
                .set_input_value("concurrent_input", value)
                .await
                .unwrap();
    ...
    let get_task = tokio::spawn(async move {
        ...
        for _ in 0..10 {
            if let Some(value) = computer_clone
                .get_input_value("concurrent_input")
                .await
                .unwrap()
            {
                values.push(value);
    ...
    timeout(Duration::from_secs(5), set_task)
        .await
        .unwrap()
        .unwrap();
    let retrieved_values = timeout(Duration::from_secs(5), get_task)
        .await
        .unwrap()
        .unwrap();
...
async fn test_cache_persistence_across_operations() {
    ...
    computer
        .set_input_value("persistent_input", test_value.clone())
        .await
        .unwrap();
...
async fn test_cache_with_complex_values() {
    ...
    let complex_values = vec![
        (
            "string_input",
            serde_json::Value::String("complex string with spaces".to_string()),
        ),
        (
            "number_input",
            serde_json::Value::Number(serde_json::Number::from(123456789)),
        ),
        (
            "float_input",
    ...
    assert_eq!(
        ...
        Some(&serde_json::Value::String(
            "complex string with spaces".to_string()
        ))
    );
    assert_eq!(
...
async fn test_cache_edge_cases() {
    ...
    computer
        ...
        .unwrap();
    assert!(computer
        .get_input_value("edge_case_input")
        .await
        .unwrap()
        .is_some());

    computer
        .clear_input_values(Some("edge_case_input"))
        .await
        .unwrap();
    assert!(computer
        .get_input_value("edge_case_input")
