...
impl Session for TestSession {
    async fn resolve_input(&self, input: &MCPServerInput) -> Result<serde_json::Value, ComputerError> {
...
async fn test_computer_with_mock_session() {
    ...
    session.set_input("username", serde_json::Value::String("test_user".to_string())).await;
    session.set_input("password", serde_json::Value::String("test_pass".to_string())).await;
    
    let mut inputs = HashMap::new();
    inputs.insert("username".to_string(), MCPServerInput::PromptString(PromptStringInput {
    ...
    inputs.insert("password".to_string(), MCPServerInput::PromptString(PromptStringInput {
...
async fn test_computer_input_value_management() {
    ...
    let set_result = computer.set_input_value(
        "test_input",
    ...
    computer.clear_input_values(Some("test_input")).await.unwrap();
}
...
async fn test_computer_multiple_servers() {
    ...
    computer.add_or_update_server(updated_server1).await.unwrap();
...
async fn test_computer_error_handling() {
    ...
    assert!(matches!(tools_result.unwrap_err(), ComputerError::InvalidState(_)));
