...
impl CliInputProvider {
    ...
    async fn read_pick(&self, prompt: &str, options: &[String]) -> InputResult<String> {
        ...
        loop {
            let input = self
                .read_string("请输入选项编号 (Please enter option number): ", false)
                .await?;
    ...
    async fn read_bool(
        ...
        loop {
            let input = self
                .read_string(
                    &format!("{} ({}/{}): ", prompt, true_label, false_label),
                    false,
                )
                .await?;
            ...
            match rule {
                ValidationRule::Regex { pattern, message } => {
                    ...
                    if !regex.is_match(value) {
                        let msg = message
                            .as_deref()
                            .unwrap_or("输入格式不正确 (Input format is incorrect)");
        ...
        let value = match &request.input_type {
            InputType::String {
                ...
                if let Some(min) = min_length {
                    if input.len() < *min {
                        return Err(InputError::ValidationFailed(format!(
                            "输入长度不能少于{}个字符 (Minimum length is {})",
                            min, min
                        )));
                    }
                }
                if let Some(max) = max_length {
                    if input.len() > *max {
                        return Err(InputError::ValidationFailed(format!(
                            "输入长度不能超过{}个字符 (Maximum length is {})",
                            max, max
                        )));
                    }
                }
            ...
            InputType::Bool {
                ...
                let bool_val = self
                    .read_bool(&prompt, true_label.as_deref(), false_label.as_deref())
                    .await?;
            ...
            InputType::FilePath { must_exist, filter } => {
                ...
                if let Some(filter) = filter {
                    if !path.ends_with(filter) {
                        return Err(InputError::ValidationFailed(format!(
                            "文件类型不匹配，期望: {} (File type mismatch, expected: {})",
                            filter, filter
                        )));
                    }
                }
        ...
        match env::var(&env_name) {
            Ok(value) => {
                ...
                let converted_value = match &request.input_type {
                    ...
                    InputType::Number { .. } => {
                        value.parse::<i64>().map(InputValue::Number).map_err(|_| {
                            InputError::ValidationFailed(format!(
                                "Invalid number in environment variable: {}",
                                env_name
                            ))
                        })?
                    }
                    InputType::Bool { .. } => {
                        ...
                        } else if lower == "false" || lower == "0" || lower == "no" || lower == "否"
                        {
                            ...
                            return Err(InputError::ValidationFailed(format!(
                                "Invalid boolean value in environment variable: {}",
                                env_name
                            )));
                        }
                    }
            ...
            Err(env::VarError::NotPresent) => {
                ...
                if let Some(default) = &request.default {
                    ...
                    Err(InputError::ValidationFailed(format!(
            ...
            Err(e) => Err(InputError::Other(format!(
