...
impl MCPServerManager {
    ...
    pub async fn start_client(&self, server_name: &str) -> Result<(), ComputerError> {
        ...
        if config.disabled() {
            return Err(ComputerError::InvalidConfiguration(
                format!("Cannot start disabled server: {}", server_name)
            ));
        }
        ...
        client.connect().await
            .map_err(|e| ComputerError::ConnectionError(format!("Failed to connect to {}: {}", server_name, e)))?;
    ...
    pub async fn stop_client(&self, server_name: &str) -> Result<(), ComputerError> {
        ...
        if let Some(ref mut c) = client {
            c.disconnect().await
                .map_err(|e| ComputerError::ConnectionError(format!("Failed to disconnect from {}: {}", server_name, e)))?;
        }
    ...
    async fn restart_server(&self, server_name: &str) -> Result<(), ComputerError> {
        ...
        let enabled = {
            let configs = self.servers_config.read().await;
            configs.get(server_name).map(|c| !c.disabled()).unwrap_or(false)
        };
    ...
    async fn refresh_tool_mapping(&self) -> Result<(), ComputerError> {
                ...
                Ok(tools) => {
                    for tool in tools {
                        ...
                        if display_name != original_tool_name {
                            ...
                            alias_map.insert(display_name.clone(), (server_name.clone(), original_tool_name.clone()));
                        }
                        ...
                        if forbidden_tools.contains(&display_name) || forbidden_tools.contains(&original_tool_name) {
        ...
        for (tool, sources) in tool_sources {
            if sources.len() > 1 {
                ...
                let suggestion = "Please use the 'alias' feature in ToolMeta to resolve conflicts. \
                    Each tool should have a unique name or alias across all servers.";
                return Err(ComputerError::InvalidConfiguration(
                    format!("Tool '{}' exists in multiple servers: {:?}\n{}", tool, sources, suggestion)
                ));
            }
    ...
    pub async fn validate_tool_call(&self, tool_name: &str, _parameters: &serde_json::Value) -> Result<(ServerName, ToolName), ComputerError> {
        // 检查工具是否可用 / Check if tool is available
        let disabled = self.disabled_tools.read().await;
        if disabled.contains(tool_name) {
            return Err(ComputerError::PermissionError(format!("Tool '{}' is disabled by configuration", tool_name)));
        }
        ...
        let server_name = server_name.ok_or_else(|| {
            ComputerError::InvalidConfiguration(format!("Tool '{}' not found in any active server", tool_name))
        })?;
    ...
    pub async fn call_tool(
        ...
        let client = {
            ...
            clients.get(server_name)
                .ok_or_else(|| ComputerError::InvalidConfiguration(
                    format!("Server '{}' for tool '{}' is not active", server_name, tool_name)
                ))?
                .clone()
        };
        ...
        let mut result = result.map_err(|e| ComputerError::ProtocolError(format!("Tool execution failed: {}", e)))?;
        ...
        if let Some(config) = config {
            if let Some(tool_meta) = self.merged_tool_meta(&config, tool_name) {
                ...
                if let Some(ref mut meta) = result.meta {
                    meta.insert(A2C_TOOL_META.to_string(), serde_json::to_value(tool_meta).unwrap());
                }
            }
        }
    ...
    pub async fn execute_tool(
        ...
        let (server_name, original_tool_name) = self.validate_tool_call(tool_name, &parameters).await?;
        self.call_tool(&server_name, &original_tool_name, parameters, timeout).await
    }
    ...
    pub async fn list_available_tools(&self) -> Vec<Tool> {
        ...
        for (display_name, server_name) in mapping.iter() {
            ...
            if let Some(client) = client {
                ...
                let original_name = alias_map.get(display_name)
