...
impl TestServer {
    async fn new() -> Self {
        ...
        Self { addr, handle, shutdown_tx: Some(shutdown_tx) }
    }
}
    ...
    F: FnMut(Payload, Client) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send>> + 'static + Send + Sync,
{
    ...
    if retries == 0 {
        panic!("Failed to receive connect event within timeout");
    }
...
async fn test_server_basic_http_endpoints() {
    ...
    let body_bytes = http_body_util::BodyExt::collect(response.into_body())
        ...
        .to_bytes();
    let text = String::from_utf8(body_bytes.to_vec()).expect("Failed to convert response to string");
    ...
    let health_body = http_body_util::BodyExt::collect(health_response.into_body())
        ...
        .expect("Failed to read response")
        .to_bytes();
    let health_json: Value = serde_json::from_str(&String::from_utf8(health_body.to_vec()).unwrap()).expect("Failed to parse JSON");
