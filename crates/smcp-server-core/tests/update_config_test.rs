...
async fn test_update_config_broadcast() {
        ...
        .on("notify:update_config", move |payload: Payload, _client| {
            ...
            async move {
                if let Payload::Text(values) = payload {
                    if let Ok(notification) = serde_json::from_value::<UpdateMCPConfigNotification>(values[0].clone()) {
                        // 验证通知内容
    ...
    // 验证Agent收到了通知
    assert!(agent_received.load(Ordering::SeqCst), 
...
async fn test_update_config_computer_not_in_office() {
    ...
    // 验证发送结果
    assert!(emit_result.is_ok(), "Should be able to emit update_config event");
...
async fn test_update_config_multiple_computers() {
    ...
    let agent_client = ClientBuilder::new(server_url.clone())
        ...
        .on("notify:update_config", move |payload: Payload, _client| {
            ...
            async move {
                if let Payload::Text(values) = payload {
                    if let Ok(_notification) = serde_json::from_value::<UpdateMCPConfigNotification>(values[0].clone()) {
    ...
    // 验证Agent收到了通知
    assert!(notification_count.load(Ordering::SeqCst), 
...
async fn test_update_config_notification_content() {
    ...
    // 存储收到的通知内容
    let received_notification = Arc::new(tokio::sync::Mutex::new(None::<UpdateMCPConfigNotification>));
