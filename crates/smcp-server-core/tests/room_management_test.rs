...
async fn test_list_room_cross_office_access_denied() {
    ...
    // 验证错误信息
    println!("Actual error message: {}", error_msg);    assert!(error_msg.contains("permission") || error_msg.contains("office"));
...
async fn test_computer_duplicate_name_rejected() {
    ...
    // 第一个Computer加入办公室
    join_office(&computer1_client, Role::Computer, "office1", "duplicate_comp").await;
    ...
    assert!(!success, "Second computer with same name should fail to join");
    
    // 验证错误信息
    if let Some(err) = result.get("err").and_then(|v| v.as_str()) {
        assert!(err.contains("already exists"), 
            "Error should contain 'already exists', got: {}", err);
    }
...
async fn test_computer_different_name_allowed() {
    ...
    // 验证加入成功
    let success = result.get("ok").and_then(|v| v.as_bool()).unwrap_or(false);
    assert!(success, "Computer with different name should succeed to join");
...
async fn test_computer_switch_room_with_same_name_allowed() {
    ...
    // Computer加入第一个房间
    join_office(&computer_client, Role::Computer, "office1", "switching_comp").await;
    ...
    // 验证切换成功
    let success = result.get("ok").and_then(|v| v.as_bool()).unwrap_or(false);
    assert!(success, "Computer should be able to switch rooms with same name");
