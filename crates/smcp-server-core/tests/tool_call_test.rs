...
async fn test_tool_call_timeout_handling() {
    ...
    assert!(result.is_ok(), "Should get an error response when Computer doesn't respond");
    ...
    // 验证错误信息
    assert!(error_msg.contains("Session not found") || error_msg.contains("error"), 
        "Expected error when Computer doesn't respond, got: {}", error_msg);
